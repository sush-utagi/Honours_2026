...
def main():
    ...
    for cls in classes:
        ...
        for tech_dir_name in sorted(os.listdir(cls_dir)):
            ...
            paths = {
                "pixel": os.path.join(raw_dir, f"fidelity_distances_pixel_{tech_raw}.json"),
                "clip": os.path.join(raw_dir, f"fidelity_distances_clip_{tech_raw}.json"),
            }
    ...
    def plot_distributions(ax, target_configs, space_key, title, xlabel, is_bottom_row):
        ...
        for i, config in enumerate(target_configs):
            ...
            ax.plot(x_fit, pdf, color=pal["curve"], linewidth=2.5, alpha=0.85, label=f"{label} ($\mu={mean_val:.3f}$)")
    ...
    # Plot Toaster (Row 0)
    plot_distributions(axes[0, 0], toaster_configs, "pixel", "Toaster – Pixel Space Conditioning Fidelity", "RMSE (Source vs Output)", False)
    plot_distributions(axes[0, 1], toaster_configs, "clip", "Toaster – CLIP Space Conditioning Fidelity", "Cosine Distance (Source vs Output)", False)
    
    # Plot Hair Drier (Row 1)
    plot_distributions(axes[1, 0], hair_drier_configs, "pixel", "Hair Drier – Pixel Space Conditioning Fidelity", "RMSE (Source vs Output)", True)
    plot_distributions(axes[1, 1], hair_drier_configs, "clip", "Hair Drier – CLIP Space Conditioning Fidelity", "Cosine Distance (Source vs Output)", True)
    
    fig.tight_layout(pad=3.0)
    
    out_path = os.path.join(base_dir, "conditioning_fidelity_comparison.png")
    fig.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"\nSuccessfully generated conditioning fidelity plot: {out_path}")
